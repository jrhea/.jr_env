...
Where [options] are:
  ...
  --replace       - Replace spaces with this string (--rep=. will return
  --Daily.Show)
...
usage if !GetOptions('verbose+'=>\$verbose, 'help'=>\$help,
'quiet'=>\$quiet,
'database=s'=>\$database, 'host=s'=>\$host, 'user=s'=>\$user,
'pass=s'=>\$pass,
'subtitle'=>\$sub, 'sublen=s'=>\$sublen, 'replace=s'=>\$rep,
'legal'=>\$legal,
'recgroup'=>\$printrecgroup,
'file'=>\$printfile, 'codec'=>\$printcodec, 'channel'=>\$printchan,
'size'=>\$printsize, 'description'=>\$printdesc, 'mpeg=i'=>\$mpeg,
'all'=>\$all, 'extension:s'=>\$extension, 'grep=s'=>\$grep,
'total'=>\$printtotal,
);
...
my $dbh =
DBI->connect("dbi:mysql:database=$database:host=$host","$user","$pass")
	|| die "Cannot connect to database ($!)\n";

my $sql = 'SELECT title, subtitle, recgroup, chanid, starttime, endtime, description FROM recorded WHERE basename=?';
...
for my $file (@files) {
	...
	my ($title, $subtitle, $recgroup, $chanid, $start, $end, $description) = $sth->fetchrow_array;
	...
	if ($mpeg || $printcodec || $printsize || $printtotal) {
		...
		# This is a bit of a hack, but recording from an ivtv card,
		# and then transcoding it works.
		$codec = $gbh >= 1.1 ? 2 : 4;
	...
	if ($print) {
	        print "$recgroup: " if $printrecgroup;
		...
		printf " (%d mins in %.3f GB %.3f GB/h)", $mins, $gb, $gbh
		if $printsize;
		print "\n";
		print " $description\n" if $printdesc;
